...
client.on("interactionCreate", async (interaction) => {
    ...
    let tckCategory = '983011944887898172'
    let tckStaff    = '983011782559957012'
    let tckAdmin    = '983011771172409454'
    let closedcategory = '983011948713095189'
if(interaction.customId == "del_tick") {
  ...
  setTimeout(function(){
        ...
        const opener = db.get(`user${interaction.channel.id}`);
          ...
          interaction.reply({ content: `✅ **Closing your ticket now!**\n> *Please give us a review in <#983012001439698954>!*`, ephemeral: true })
  ...
  const del_row = new MessageActionRow()
          ...
          setTimeout(function(){
            interaction.channel.setParent("983011948713095189");
          ...
          const log = new MessageEmbed()
                 .setColor('#f65bad')
                .setAuthor(`Ticket-Log for: ${interaction.channel.name}`, `https://cdn.discordapp.com/emojis/853259977031417906.png?size=512`, `https://discord.gg/dpdx`)
                .setDescription(`<:Punished:933761218366304257> Closed by: **\`${interaction.user.tag}\`**`)
                .setFooter(`The Transcript is attacthed to this message above!`, client.user.displayAvatarURL())
                const attachment = await discordTranscripts.createTranscript(interaction.channel);

                

                client.channels.cache.get("983011968644444170").send({ content: `<@`+interaction.user.id+`> **You Closed a ticket!**`, embeds: [log], files: [attachment]})
  ...
  if (interaction.customId == "azu_rules_codes") {
  if(!client.config.developers.includes(interaction.user.id)) return interaction.reply({ content: `❌ **Your ID isn't found in the CLIENT_OWNERS config!**`, ephemeral: true })
  interaction.message.edit({ embeds: [client.rulesEmbed]})
  interaction.reply({ content: `✅ **Reloaded the Rules-Embed!**`, ephemeral: true })
  }
  if (interaction.customId == "azu_reload_premium_codes") {
  if(!client.config.developers.includes(interaction.user.id)) return interaction.reply({ content: `❌ **Your ID isn't found in the CLIENT_OWNERS config!**`, ephemeral: true })
  interaction.message.edit({ embeds: [client.PremiumcodesEmbed]})
  interaction.reply({ content: `✅ **Reloaded the Premium-Codes-Embed!**`, ephemeral: true })
  }
    if (interaction.isSelectMenu()) {
      ...
      const embed = new MessageEmbed()
        .setAuthor(`${interaction.user.username} Claimed this ticket!`, interaction.user.displayAvatarURL())
          .setColor('#f65bad')
        .setFooter(`Ticketing by dpdxgamer.ga`, interaction.guild.iconURL())

        const embed2 = new MessageEmbed()
          .setColor('#f65bad')
        .setAuthor(`${interaction.user.tag} ⚡ Team Member / Supporter`, interaction.user.displayAvatarURL(), `https://discord.gg/dpdx`)
