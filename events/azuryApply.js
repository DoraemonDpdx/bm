...
let tckCategory = '983011928655929414'
    let tckStaff    = '983011771172409454'
    ...
    if(interaction.customId == `azu_apply`) {
      ...
      if(interaction.member.roles.cache.some(x => x.id == "983011782559957012")) return interaction.reply({ content: `❌ **You already are a staff member!**`, ephemeral: true })
    const tckEmb = new MessageEmbed()
          .setAuthor({ name: `APPLICATION TICKET`, iconURL: `${client.user.displayAvatarURL()}` })
          .setColor('#f65bad')
          .setFooter(`Made with 💖 by Dpdx Hangout`) 
      ...
      if(!interaction.member.roles.cache.some(x => x.id == tckStaff)) return interaction.reply({ content: `🔐・You must have the **<@&${tckStaff}>** role to be able to use this option!`, ephemeral: true })
          ...
          user.roles.add("983011783444947054")
          user.roles.add("983011806521999371")
      ...
      interaction.reply({ content: `🔒 Closing within the next few seconds..`, ephemeral: true })
          ...
          const log = new MessageEmbed()
                 .setColor('#f65bad')
                .setAuthor(`Ticket-Log for: ${interaction.channel.name}`, `https://cdn.discordapp.com/emojis/853259977031417906.png?size=512`, `https://dsc.gg/dpdx`)
