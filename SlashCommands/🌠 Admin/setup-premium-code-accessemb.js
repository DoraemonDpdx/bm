...
module.exports = {
    name: "setup-premiumcodesembed",
    description: "👑 Setup the Premium Code-Packages Embed",
    type: 'CHAT_INPUT',
    options: [
      {
        name: "channel",
        description: "Select a channel you want to send the Premium  Code-Packages Embed to",
        type: "CHANNEL",
        required: true,
      }
    ], 
    run: async (client, interaction, args) => {
      ...
      const row = new MessageActionRow()
			.addComponents(
			  new MessageButton()
        .setCustomId(`azu_reload_premium_codes`)
        .setEmoji(`870013196591837214`)
        .setStyle(`DANGER`)
			);
      ch.send({ content: ` `, embeds: [client.PremiumcodesEmbed], components: [row] });
      return msg.edit({ content: `💖 The codes embed has successfully been sent! View it here: ${ch}
Boosters And Donaters Enjoy! ` })
    },
};
